...
geneVisualization <- function ( scDEsubs.out, 
                ...
                verbose=TRUE)    
{
    ...
    edgeList <- scDEsubs.out[['edgeList']]
    DEgenes  <- scDEsubs.out[['DEgenes']]
    org      <- scDEsubs.out[['org']]
    nomen    <- scDEsubs.out[['mRNAnomenclature']]
...
subpathwayVisualization <- function( scDEsubs.out, 
                                references='', 
                                submethod, 
                                subname,
                                ...
                                verbose=TRUE )
{
    ...
    {
        out[[i]] <- .subpathwayVisualization(scDEsubs.out=scDEsubs.out,
...
.subpathwayVisualization <- function( scDEsubs.out, reference, submethod, 
                                    ...
                                    outfile, export, verbose)
{
    ...
    subpathway.nodelist <- scDEsubs.out[[submethodName]][[subname]]
...
#
organismVisualization <- function( scDEsubs.out, 
                            references='', 
                            topSubs=10, 
                            topTerms=20, 
                            ...
                            verbose=TRUE )
{
    ...
    for ( i in seq_len(length(references)) )
    {
        out[[i]] <- .organismVisualization(scDEsubs.out=scDEsubs.out,
...
.organismVisualization <- function( scDEsubs.out, references, topSubs, topTerms,
