                                ...
                                pathways='all')
{
    ...
    if ( mRNAnomenclature != 'entrezgene' ) 
    {
        ...
        file  <- system.file(file, package='scDEsubs')
        load(file, e <- new.env())
        ...
        xlib <- lib[, 1]
        names(xlib) <- lib[, 2]
        uniq.xlib <- make.unique(as.character(xlib[rownames(mRNAexpr)]), sep = ".")
        rownames(mRNAexpr) <- uniq.xlib
    }

    # Create pathway graph
    file  <- system.file('extdata//Data//edgeLists.RData', package='scDEsubs')
    load(file, e <- new.env())
...
.pruneNetwork <- function(   edgeList, mRNAexpr, DEGchoice, classes, 
                            ...
                            CORtool, rankedList=NULL, grouping=NA, ...)
{
    argList<-list(...)
    ...
    if ( is.null(rankedList) )
    {
        ...
        genes <- .DEanalysis( count.matrix=mRNAexpr, DEGchoice=DEGchoice, 
                            classes=classes, org = org, grouping=grouping,
                            DEGthresh=DEGthresh,
                            mRNAnomenclature = argList[[1]] )
    }
...
.pruneEdges   <- function( edgeList, mRNAexpr, thr, CORtool )
{
    ...
    {
        CR[i] <- cor(as.numeric(exprEdgelistSource[i,]),
                     as.numeric(exprEdgelistDestin[i,]),  method = CORtool )
    }
    # saveRDS(CR, file = "pvals/CR.rds")
    #####Scale between 0-1 
    # CR<-as.numeric(scale(m, center = mins, scale = maxs - mins))
    # m<-as.matrix(CR)
    # maxs <- apply(m, 2, max)
    # mins <- apply(m, 2, min)
    
    #####
    # saveRDS(CR, file = "pvals/CR_scaled.rds")
